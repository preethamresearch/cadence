...
// The static build has no backend, so replay is the only mode that can
// work there — force it rather than let the live button dead-end.
const STATIC_BUILD = Boolean(window.__CADENCE_STATIC__);
if (STATIC_BUILD || new URLSearchParams(location.search).has("replay")) {
  ...
  if (STATIC_BUILD) {
    els.micButton.title =
      "Live mode needs the cadence server and a Gemini key — run it locally.";
  }
